...
urlpatterns = [
    # API URLs
    path('api/', include([
        path('login-set-cookie/', views.get_csrf_token, name='CSRFtoken'),
        path('login/', views.apiLogin, name='api_login'),
        path('register/', views.apiRegisterUser, name='api_register'),
    ])),
    path('user/', include([
        path('<int:id>/profile/',
             views.UserProfileView.as_view(), name='user_profile'),
        path('<int:id>/home/', views.coursesList, name='courses_list'),
    ])),
